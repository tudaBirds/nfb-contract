...
// File contracts/Commemorative.sol

pragma solidity 0.8.9;

contract Commemorative is ERC1155, Ownable, ReentrancyGuard, ControlledAccess {
    using Strings for uint256; 
    /** Contract Functionality Variables */
    uint256 public constant maxMintPerAddress = 10;
    uint256 public maxMintPerTx = 10;
    uint256 public mintPrice = 0.1 ether;
    uint256 public totalSize = 1000;
    ...
    string public baseTokenURI = "https://nft.tudabirds.io/api/cmm/";
    string public hiddenMetadataUri = "https://nft.tudabirds.io/api/cmm/hidden";
    ...
    modifier mintCompliance(address _sender, uint256 _id, uint256 _quantity, uint256 _maxPerAddress, uint256 _maxMintPerTx) {
        require(saleActive, "Sale is not live.");
        require(totalSupply[_id] + _quantity <= totalSize, "Max supply reached");
        require(_quantity >= 0 && _quantity <= _maxMintPerTx, "Invalid mint amount per transaction"); 
        require(numberMinted(_sender, _id) + _quantity <= _maxPerAddress, "Can not mint this many");
        _;
    }
    ...
    function mint(uint256 id, uint256 quantity)
        ...
        callerIsUser
        nonReentrant
        mintCompliance(msg.sender, id, quantity, maxMintPerAddress, maxMintPerTx) /** Mint Compliance for Public Sale */
    {
    ...
    function mintBatch(uint256[] memory ids, uint256[] memory amounts)
        ...
        nonReentrant
    {
        require(ids.length == amounts.length, "Parameter length does not match");
        ...
        refundIfOver(totalAmounts * mintPrice);
    }

    function mintFromPacker(uint256 id, uint256 quantity, address to)
        external
        isPackerContract
        nonReentrant
        mintCompliance(to, id, quantity, maxMintPerAddress, maxMintPerTx) /** Mint Compliance for Public Sale */
    {
    ...
    function numberMinted(address owner, uint256 id) public view returns (uint256) {
    ...
    function tokenURI(uint256 _tokenId)
        ...
        returns (string memory)
    {
        if (keccak256(abi.encodePacked(baseTokenURI)) == keccak256(abi.encodePacked(""))) {
        ...
        return
            bytes(baseTokenURI).length > 0
                ? string(
                    abi.encodePacked(baseTokenURI, _tokenId.toString(), uriSuffix)
                )
                : "";
    }
    ...
    function ownerMint(address to, uint256 id, uint256 quantity) public 
        callerIsUser 
        onlyOwner 
        mintCompliance(to, id, quantity, maxMintPerAddress, maxMintPerTx)
    {
        _mint(to, id, quantity, "");
        totalSupply[id] += quantity;
    }

    function ownerMintBatch(address[] memory to, uint256 id, uint256 quantity) public 
        callerIsUser 
        onlyOwner 
    {
        for(uint256 i = 0; i < to.length; i ++ ) 
            _mint(to[i], id, quantity, "");
    }
